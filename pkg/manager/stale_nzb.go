...
// clearSupersededRunID: it holds the same singleton slot a sweep/fix/clear
// run does for the duration of a stale-NZB cleanup, since deletion reads and
// mutates storage a concurrent sweep could be probing/saving at the same
...
func (r *Repair) CleanupStaleNZBs(ctx context.Context, entryIDs []string, orphanIDs []string) (StaleNZBCleanupResult, error) {
	...
	// duration of this pass: it reads and deletes entries a concurrent sweep
