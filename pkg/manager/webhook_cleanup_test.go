...
func TestHandleArrWebhookCleanup_LibraryMatchTiers(t *testing.T) {
	const (
		aristocatsEntry = "The.AristoCats.1970.BluRay.1080p.REMUX.AVC.DTS-HD.MA.5.1-LEGi0N"
		aristocatsFile  = "The.AristoCats.1970.BluRay.1080p.REMUX.AVC.DTS-HD.MA.5.1-LEGi0N.mkv"
		successionEntry = "Succession.S02E07.iTALiAN.1080p.WEB.H264-CHEOPE"
		successionFile  = "Succession.S02E07.iTALiAN.1080p.WEB.H264-CHEOPE.mkv"
		...
		renamedAristocatsFile = "The Aristocats (1970) {imdb-tt0065421} [Remux-1080p][DTS-HD MA 5.1][AVC]-LEGi0N.mkv"
	)
	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func(t *testing.T, m *Manager) {
				addTestEntry(t, m, twoFileEntry("aristocats-hash", aristocatsEntry, aristocatsFile, old, false, config.ProtocolNZB))
				m.arrLibraryMap.record(aristocatsEntry, aristocatsFile,
					"/downloads/nzb/"+aristocatsEntry+"/"+aristocatsFile,
					"/mnt/rclone/__all__/"+aristocatsEntry+"/"+aristocatsFile,
				)
			},
			event: ArrWebhookEvent{
				DeleteReason: "upgrade",
				FileName:     renamedAristocatsFile,
				LibraryPath:  "/downloads/nzb/" + aristocatsEntry + "/" + aristocatsFile,
			},
			wantAction:    "file-removed",
			wantMatchedBy: "symlink_path",
			wantEntryName: aristocatsEntry,
		},
		{
			name: "movie: renamed reported path falls back to a recorded basename match",
			setup: func(t *testing.T, m *Manager) {
				addTestEntry(t, m, twoFileEntry("aristocats-hash-2", aristocatsEntry, aristocatsFile, old, false, config.ProtocolNZB))
				m.arrLibraryMap.record(aristocatsEntry, aristocatsFile,
					"/downloads/nzb/"+aristocatsEntry+"/"+aristocatsFile,
				)
			},
			event: ArrWebhookEvent{
				...
				LibraryPath: "/media/Movies/The Aristocats (1970) {imdb-tt0065421}/" + aristocatsFile,
			},
			wantAction:    "file-removed",
		...
		},
		{
			...
			setup: func(t *testing.T, m *Manager) {
				addTestEntry(t, m, twoFileEntry("succession-hash", successionEntry, successionFile, old, false, config.ProtocolNZB))
			},
			event: ArrWebhookEvent{
				DeleteReason: "manual",
				// The renamed library file Arr actually reports - unmatchable
				// by path or basename against anything decypharr recorded.
				FileName:    "Succession - S02E07 - Return WEBDL-1080p.mkv",
				LibraryPath: "/media/TV/Succession/Season 02/Succession - S02E07 - Return WEBDL-1080p.mkv",
				SceneName:   successionEntry,
			},
			wantAction:    "file-removed",
			wantMatchedBy: "scene_name",
			wantEntryName: successionEntry,
		},
		{
