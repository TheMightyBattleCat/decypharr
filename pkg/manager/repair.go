...
// decided by AutoRepair. With no active sweep this is a no-op.
func (r *Repair) stopActiveSweep() {
	r.mu.Lock()
	cancel := r.cancelRun
	id := r.activeRunID
	...
	r.logger.Info().Str("run_id", id).Msg("Repair sweep stop schedule fired; stopping sweep")
	if stopFunc != nil {
		stopFunc()
	}
	cancel()
